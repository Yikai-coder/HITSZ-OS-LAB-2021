...
found:
  ...
  // printf("Try alloc trapframe\n");
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    release(&p->lock);
    return 0;
  }
  // printf("Finish alloc trapframe\n");
  // 创建进程内核页表
...
proc_freepagetable(pagetable_t pagetable, uint64 sz)
{
  ...
  // printf("uvmfree in proc_freepagetable\n");
  // vmprint(pagetable);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if(PGROUNDUP(sz) < PGROUNDUP(sz-n)){
      ...
      // printf("Unmap user pgtbl in kernel pgtbl\n");
